...
| cluster_auth_preference | Configuration for the clusters auth preferences. |
...
spec:
  ...
  # Possible values: "on", "off", "otp", "u2f", "webauthn" and "optional".
  ...
  # Deprecated by Webauthn: U2F are settings for the u2f device.
  u2f:
    # app_id returns the application ID for universal second factor.
    ...
    facets: ["https://example.com:8000"]
    # Contains the trusted attestation CAs for U2F devices.
    device_attestation_cas: ["-----BEGIN CERTIFICATE-----\nfake certificate\n-----END CERTIFICATE-----"]
version: v2
```
