...
import { Labels, hasMatchingLabels } from '../ManualDeploy/ManualDeploy';
...
export function AutoDeploy({ toggleDeployMethod }: DeployServiceProp) {
  const { emitErrorEvent, nextStep, emitEvent } = useDiscover();
  ...
  const agentMeta = {
    ...
    db: {
      ...
      type: 'rds',
      protocol: 'postgres' as any,
      labels: [
        { name: 'region', value: 'us-east-1' },
        { name: 'account-id', value: 'account-id-1234' },
      ],
      names: [],
      users: [],
      hostname: 'some-hostname',
      aws: {
        resourceId: 'resource-id-1234',
        region: 'us-east-1',
  ...
  function handleDeploy(validator: Validator) {
    ...
    integrationService
      .deployAwsOidcService(agent.integrationName, {
        ...
        region: agent.db.aws?.rds?.region,
        subnetIds: agent.db.aws?.rds?.subnets,
        taskRoleArn,
          ...
          <>
            ...
            {isProcessing && (
              <DeployHints
                ...
                resourceName={agentMeta.db.name}
...
const CreateAccessRole = ({
  ...
  return (
    <StyledBox mb={5}>
      ...
      <Text mb={2}>
        Then open{' '}
        <Link href="https://aws.amazon.com/cloudshell/" target="_blank">
          Amazon CloudShell
        </Link>{' '}
  ...
  if (showHint && !result) {
    ...
    hint = (
      <WaitingInfo>
        ...
        cluster. We'll update this progress once detected.
      </WaitingInfo>
    );
  }
